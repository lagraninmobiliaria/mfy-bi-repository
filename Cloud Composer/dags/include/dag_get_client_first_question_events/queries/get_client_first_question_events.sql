...
FROM (
    ...
    FROM `infrastructure-lgi.{{DATASET_MUDATA_RAW}}.question_events` question_events
    WHERE  
        NOT EXISTS(
            SELECT 
                *
            FROM `infrastructure-lgi.{{DATASET_MUDATA_CURATED}}.clients_first_question_events` clients_first_question_events
            WHERE 
